...
def index():
    name = request.args.get("name", "")          # Record name
    step = int(request.args.get("step", 0))      # Replay starting step
    speed = int(request.args.get("speed", 2))    # Replay speed (0~5)
