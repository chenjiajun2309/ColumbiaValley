...
class Agent:
    ...
    def think(self, status, agents):
        ...
        if (plan["describe"] == "sleeping" or "sleep" in plan["describe"]) and self.is_awake():
    ...
    def make_schedule(self):
        if not self.schedule.scheduled():
            ...
            if self.associate.index.nodes_num > 0:
                ...
                focus = [
                    f"{self.name}'s plan on {utils.get_timer().daily_format()}.", 
            ...
            thought = "This is {}'s plan on {}: {}".format(
    ...
    def reflect(self):
        ...
        if self.chats:
            ...
            _add_thought(f"Regarding {self.name}'s plan: {thought}", evidence)
    ...
    def _chat_with(self, other, focus):
        ...
        for i in range(self.chat_iter):
            ...
            if i > 0:
                # For the initiating agent, from the 2nd round, check for repetition
                end = self.completion(
                ...
                # For the initiating agent, from the 2nd round, check if topic has ended
            ...
            if i > 0:
                # For the responding agent, from the 2nd round, check for repetition
                end = self.completion(
            ...
            # For the responding agent, from the 1st round, check if topic has ended
