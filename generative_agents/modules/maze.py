...
class Maze:
    ...
    def find_path(self, src_coord, dst_coord):
        ...
        while frontier and distance_map[dst_coord[1]][dst_coord[0]] == 0:
            new_frontier = []
            for f in frontier:
                for c in self.get_around(f):
                    if c in visited:
                        continue
                    if distance_map[c[1]][c[0]] != 0:
                        continue
                    distance_map[c[1]][c[0]] = distance_map[f[1]][f[0]] + 1
        ...
        if distance_map[dst_coord[1]][dst_coord[0]] == 0:
            return []

        step = distance_map[dst_coord[1]][dst_coord[0]]
        path = [tuple(dst_coord)]
        while step > 1:
            for c in self.get_around(path[-1], no_collision=False):
                if distance_map[c[1]][c[0]] == step - 1:
                    path.append(c)
                    break
            step -= 1
    ...
    def get_around(self, coord, no_collision=True):
        ...
        neighbours = []
        for c in candidates:
            if not (0 <= c[0] < self.maze_width and 0 <= c[1] < self.maze_height):
                continue
            if no_collision and self.tile_at(c).collision:
                continue
            neighbours.append(c)
        return neighbours
    ...
    def get_address_tiles(self, address):
        ...
        # If address not found, return a random address tiles set as fallback
        if self.address_tiles:
            return random.choice(list(self.address_tiles.values()))
        # If no address tiles exist, return empty set
        return set()
